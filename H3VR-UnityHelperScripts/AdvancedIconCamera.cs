...
namespace MeatKit
{
    ...
    public class AdvancedIconCamera : MonoBehaviour
    {
        ...
        { 
            ...
            Vector3 camRot = thisCamera.transform.eulerAngles;

            float xExtends = Mathf.Abs(Mathf.Cos(camRot.y * Mathf.Deg2Rad)) * BoundingBoxSize.x + Mathf.Abs(Mathf.Sin(camRot.y * Mathf.Deg2Rad)) * BoundingBoxSize.z;
            float yExtends = Mathf.Abs(Mathf.Cos(camRot.x * Mathf.Deg2Rad)) * BoundingBoxSize.y + Mathf.Abs(Mathf.Sin(camRot.x * Mathf.Deg2Rad) * Mathf.Cos(camRot.y * Mathf.Deg2Rad)) * BoundingBoxSize.z + Mathf.Abs(Mathf.Sin(camRot.x * Mathf.Deg2Rad) * Mathf.Sin(camRot.y * Mathf.Deg2Rad)) * BoundingBoxSize.x;

            float XPicture = Mathf.Abs(Mathf.Cos(camRot.z * Mathf.Deg2Rad)) * xExtends + Mathf.Abs(Mathf.Sin(camRot.z * Mathf.Deg2Rad)) * yExtends;
            float YPicture = Mathf.Abs(Mathf.Cos(camRot.z * Mathf.Deg2Rad)) * yExtends + Mathf.Abs(Mathf.Sin(camRot.z * Mathf.Deg2Rad)) * xExtends;

            float size = XPicture > YPicture ? (1f + Oversize) * XPicture / 2f : (1f + Oversize) * YPicture / 2f;
